  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (res.status === "failed") {
        setErrMsg({ status: "error", message: res.message });
      } else {
        // Uncomment if you have a Redux action to update user info
        // dispatch(Login(newData));
  
        setErrMsg({ status: "success", message: res.message });
  
        // Redirect the user to the job listing page after a successful upload
        Navigate('/find-jobs');
      }
    } catch (error) {
      console.error(error);
      toast.error(error.response?.data?.message || "Failed to upload job");
    } finally {
      setIsLoading(false);
    }
